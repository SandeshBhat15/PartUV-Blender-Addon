...
# order sets the enum/ui order - PartUV is main (SIGGRAPH Asia 2025)
_engines = []

# partuv is optional, some builds ship without its folder - but we want it FIRST when present
if importlib.util.find_spec(f"{__name__}.partuv") is not None:
    from . import partuv  # noqa: E402

    _engines.append(partuv.ENGINE)

_engines.extend([optcuts.ENGINE, xatlas.ENGINE])
...
def installed_engines():
    ...
    if _installed_cache is None:
        try:
            ...
            # Safe fallback during draw when preferences not ready
            _installed_cache = []
