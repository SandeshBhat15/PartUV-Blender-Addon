...
class InputExporter:
    ...
    def _triangulate_mesh(self, obj, unwrap, path, props):
        ...
        ngon_dict = {}
        # To match PartUV demo exactly, let PartUV's own preprocess handle triangulation
        # for PartUV engine. Only triangulate here for xatlas/optcuts which need it.
        do_triangulate_here = not (hasattr(self.engine, 'id') and self.engine.id == "PARTUV")
        for face_idx, face in enumerate(bm.faces):
            if len(face.edges) > 3:
                if do_triangulate_here:
