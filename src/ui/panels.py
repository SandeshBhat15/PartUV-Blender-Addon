...
class UVGAMI_PT_main(bpy.types.Panel):
    ...
    def draw_header(self, context):
        try:
            props = getattr(context.scene, "uvgami", None)
            if props is None:
    ...
    def draw(self, context):
        try:
            ...
            return
        # Show One-Click auto-install progress on MAIN page (visible even when engine installed)
        try:
            from .oneclick import _state as _oc_state
            if _oc_state.get("running"):
                b = self.layout.box()
                b.label(text="Downloading PartUV AI...", icon='IMPORT')
                b.label(text=_oc_state.get("progress") or "Starting...", icon='TIME')
                try:
                    from ..engines.install_task import task_state as _ts
                    done = _ts.get("bytes_done") or 0
                    total = _ts.get("bytes_total")
                    phase = _ts.get("phase") or ""
                    if total:
                        b.progress(factor=done/total, type='BAR', text=f"{phase} {done/1024/1024:.1f}/{total/1024/1024:.1f} MB {done/total*100:.0f}%")
                    elif done:
                        b.label(text=f"{phase}: {done/1024/1024:.1f} MB")
                    else:
                        b.label(text=phase)
                except: pass
                for line in _oc_state.get("log", [])[-2:]:
                    b.label(text=line[:64])
        except: pass
        ...
        if engine is None:
            try:
                ...
                self.layout.label(text="Engine not ready", icon='ERROR')
            # Also show One-Click progress here when no engine
            try:
                from .oneclick import _state as _oc2
                if _oc2.get("running"):
                    b = self.layout.box()
                    b.label(text=_oc2.get("progress"), icon='TIME')
            except: pass
            return
